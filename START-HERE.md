...
**草案（先设计未实现）：** M15 限流+配置热更新 ｜ M16 长期记忆+文件链路 ｜ M17 企业基建 ｜ M18 定时任务+MCP双层JSON ｜ M19 本地编码 Agent(AI Coding)

> 想继续往前做：优先级最高的是 **M15（请求级限流 + 配置热更新，低成本高价值）**；最有意思的是 **M19（本地编码 Agent）**。草案总览表见 `LEARNING-PLAN.md`。
